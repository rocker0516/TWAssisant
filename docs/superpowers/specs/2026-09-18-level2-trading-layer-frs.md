...
## 5. Phase 1 基準策略（Baseline v0）

參數表（**凍結；任何改動＝policy_version 變更＋戰績歸零**）：

| 參數 | 值 | 說明 |
|---|---|---|
| 主組合 P5 | 每 5 個交易日再平衡 | 用 5D pct_rank |
| K_in / K_hold | 20 / 60 | 買進門檻 rank ≤ 20；既有持股 rank ≤ 60 續抱（緩衝降換手） |
| 權重 | 等權，目標 5%／檔 | 零股計算，現金殘留滾存 |
| 防禦出場 | 持股 1D pct_rank ≤ 0.2 → 次日出場 | 不等再平衡日；edge 是避弱，把它寫成規則 |
| 對照組合 P1 | 每日再平衡 1D Top-20 等權 | 同成本模型，驗證 1D 訊號成本後淨 edge |
