...
- Produces: `routes_level1.load_validation() -> dict | None`；`GET /api/level1/validation` 回傳 `{first_test, periods, model_version, horizons: {hz: {ladder: {model: {dev_oos: cell, holdout: cell}}, quantiles: {dev_oos: [10], holdout: [10]}}}}`，cell = `{mean_ic, icir, monotonicity, n_days, top20_excess_pct, top20_day_win_rate}`
...
def test_validation_endpoint_shape(client):
    ...
    assert len(h5["quantiles"]["dev_oos"]) == 10
    assert len(h5["quantiles"]["holdout"]) == 10
...
def load_validation() -> dict | None:
    ...
    out = {
        ...
        "model_version": CURRENT_MODEL_VERSION, "horizons": {},
    }
    for hz in _SHOW_HORIZONS:
        ...
        out["horizons"][hz] = {
            ...
            "quantiles": {"dev_oos": h["lgbm"]["dev_oos"]["quantile_mean_pct"],
                          "holdout": h["lgbm"]["holdout"]["quantile_mean_pct"]},
        }
...
export type Level1HorizonValidation = {
  ...
  quantiles: { dev_oos: number[]; holdout: number[] };
};
...
**Interfaces:**
- Consumes: Task 3 的 `Level1Validation`、既有 `Level1Performance`
...
const METRICS: {
  ...
  { key: "mean_ic", label: "Rank IC",
    tip: "每日 Spearman(score, 實際 N 日報酬) 的平均；0.05 以上即具實用排序力",
    fmt: (v) => v.toFixed(4) },
  ...
  { key: "top20_excess_pct", label: "Top-20 超額",
    tip: "Top-20 日均報酬 − 池內全體日均（百分點）",
    fmt: (v) => `${v >= 0 ? "+" : ""}${v.toFixed(2)}pp` },
  { key: "top20_day_win_rate", label: "日勝率",
    tip: "Top-20 贏過池內均值的日子占比", fmt: (v) => `${(v * 100).toFixed(1)}%` },
  { key: "n_days", label: "評估天數", tip: "", fmt: (v) => String(v) },
];

function QuantileBars({ title, values, mono }: {
  title: string; values: number[]; mono: number;
}) {
  ...
  return (
    <div className="min-w-[260px] flex-1">
      ...
      <div className="mt-1 text-[11px] text-gray-500">
        {mono < 0.5
          ? "edge 主要來自避開最弱分位，頂部拉抬幅度有限"
          : `分數越高實際越強，單調性 ${mono.toFixed(2)}`}
      </div>
    </div>
  );
}
      ...
      <section className="space-y-3">
        <div>
          ...
          <p className="text-xs text-gray-500">
            驗證母體＝可交易 U_t（與榜單同一池），預測母體約 599 檔／日。
          </p>
        </div>
        ...
        {hv && (
          <div className="flex flex-wrap gap-4 rounded-lg border border-gray-800 bg-gray-900/60 p-3">
            <QuantileBars title={`十分位實際報酬（dev，${horizon}D，%）`}
                          values={hv.quantiles.dev_oos}
                          mono={hv.ladder.lgbm.dev_oos.monotonicity} />
            <QuantileBars title={`十分位實際報酬（holdout，${horizon}D，%）`}
                          values={hv.quantiles.holdout}
                          mono={hv.ladder.lgbm.holdout.monotonicity} />
          </div>
        )}
...
4. 體檢籤 A1 六卡 dev/holdout 並排；A2 兩張分位圖（5D holdout 非單調如實）；
   A3 階梯表 lgbm 高亮、random ≈ 0、動能為負
5. B 區顯示空狀態說明（v2 實績未成熟）
6. `read_console_messages` 零錯誤
